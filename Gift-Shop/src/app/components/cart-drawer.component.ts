...
@Component({
  ...
  template: `
    ...
    <aside id="cart-drawer" class="cart" [class.open]="open">
      ...
      <div class="cart-body">
        ...
        <!-- ── Order placed success banner (dismissible) ── -->
        <ng-container *ngIf="orderPlacedSuccess && placedOrder">
          <div class="order-placed-banner">
            <button class="banner-dismiss" type="button" (click)="dismissBanner()" title="Dismiss">✕</button>
        ...
        <!-- ── Cart / checkout (always shown so user can place another order) ── -->
        <ng-container *ngIf="items.length > 0; else emptyState">
          ...
          <div class="payment-details-section" *ngIf="paymentDetails().length > 0">
            <h4>Payment Information</h4>
            <ul>
              <li *ngFor="let pd of paymentDetails()">
                <strong>{{ pd.key }}:</strong> {{ pd.value }}
              </li>
            </ul>
          </div>
          ...
          <form class="checkout-section" (submit)="placeOrder($event)">
            ...
            <!-- Payment method selection – only UPI and Online -->
            <div class="payment-method-group">
              ...
              <div class="method-options">
                ...
                <label class="method-option" [class.selected]="paymentMethod() === 'Online'">
                  <input type="radio" name="paymentMethod" value="Online" (change)="onMethodChange('Online')" />
                  <span>Online Payment</span>
                </label>
              </div>
            </div>

            <!-- Transaction ID input only when UPI is selected -->
            ...
            <!-- Optional note for Online payment -->
            <div class="form-group" *ngIf="paymentMethod() === 'Online'">
              <p class="online-note">You'll be redirected to a secure payment page after placing your order.</p>
            </div>
  ...
  styles: [`
    ...
    /* ─── Order placed banner (dismissible) ─── */
    .order-placed-banner{position:relative;background:linear-gradient(135deg,rgba(136,173,53,0.08),rgba(136,173,53,0.03));border:1.5px solid rgba(136,173,53,0.3);border-radius:16px;padding:1.5rem 1.25rem 1.25rem;margin-bottom:1.25rem;text-align:center}
    .banner-dismiss{position:absolute;top:0.75rem;right:0.75rem;background:rgba(0,0,0,0.06);border:none;width:28px;height:28px;border-radius:50%;font-size:0.85rem;display:flex;align-items:center;justify-content:center;cursor:pointer;color:#666}
    .banner-dismiss:hover{background:rgba(0,0,0,0.12)}
    .order-placed-banner .order-placed-icon{font-size:2.5rem;margin-bottom:0.5rem}
    .order-placed-banner h4{font-family:var(--font-display);color:var(--color-charcoal);font-size:1rem;margin-bottom:0.3rem}
    .placed-copy{font-size:0.85rem;color:var(--color-body);margin-bottom:0.5rem}
    .order-id-box{background:var(--color-sage);border-radius:10px;padding:0.75rem 1rem;margin:0.75rem 0;font-family:var(--font-ui);text-align:left}
    .order-id-label{font-size:0.75rem;color:var(--color-body);margin-bottom:0.2rem;font-weight:500}
    .order-id-value{font-size:1.1rem;font-weight:700;color:var(--color-primary);letter-spacing:0.04em}
    .order-id-hint{font-size:0.72rem;color:var(--color-body);margin-top:0.3rem}
    /* ─── Cart ─── */
    .cart-empty{text-align:center;padding:3rem 1rem;color:var(--color-body)}
    .cart-empty-icon{font-size:3rem;margin-bottom:1rem}
    .cart-empty h4{font-family:var(--font-display);color:var(--color-charcoal);font-size:1.1rem}
    ...
    .payment-details-section{margin-top:1rem;margin-bottom:1rem;padding:0.75rem;background:rgba(136,173,53,0.05);border-radius:10px}
    .payment-details-section h4{font-family:var(--font-ui);font-size:0.85rem;color:var(--color-charcoal);margin-bottom:0.5rem}
    .payment-details-section ul{list-style:none;padding:0;margin:0}
    .payment-details-section li{font-size:0.82rem;color:var(--color-body);padding:0.25rem 0;border-bottom:1px solid rgba(0,0,0,0.04)}
    .online-note{font-size:0.82rem;color:var(--color-body);margin-top:0.25rem}
...
export class CartDrawerComponent {
  ...
  constructor(private state: AppStateService) {
    ...
    // Payment details (new) – loaded from AppStateService
    this.state.paymentDetails$.subscribe(details => {
  ...
  onMethodChange(method: string) {
    ...
    // Clear transaction ID when switching methods
  ...
  placeOrder(e: Event) {
    ...
    if (method === 'UPI') {
      txnId = this.transactionId().trim();
      if (!txnId) {
        this.error.set('Transaction ID is required for UPI payment.');
        return;
      }
    } else if (method === 'Online') {
      // No transaction ID needed; optionally leave undefined
      txnId = undefined;
    } else {
